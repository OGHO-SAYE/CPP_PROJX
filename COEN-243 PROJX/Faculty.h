...
public:
	...
	Faculty()
	{
		// Local variables to store student data read from file
		string firstName;
		string lastName;
		string id;
		string dob;
		...
		countECE = countME = countCE = 0;

		// Opens the file for reading ECE students information
		ifstream readF;
		readF.open("student_elec_comp.txt");

		// Checks if the file unable to open for reading display's error message and stop
		if (!readF)
		{
			cout << "\n ERROR: Unable to open the file student_elec_comp.txt for reading.";
			exit(0);
		}// End of if condition
		...
		// Dynamically allocates memory for ECE pointer type object array of size countECE
		ECE = new Student[countECE];

		// Loops till number of records in file
		for (int c = 0; c < countECE; c++)
		{
			// Reads a record
			readF >> firstName >> lastName >> id >> dob >> gpa >> startYear >> completedCredit >> program;

			// Calls the setter function to set data for current object
			ECE[c].setFirstName(firstName);
			ECE[c].setLastName(lastName);
			ECE[c].setID(id);
			ECE[c].setDOB(dob);
			ECE[c].setGPA(gpa);
			ECE[c].setStartYear(startYear);
			ECE[c].setCompletedCredit(completedCredit);
			ECE[c].setProgram(program);
		}// End of while loop

		// Close the file
		readF.close();

		// Opens the file for reading ME students information
		readF.open("student_mech.txt");

		// Checks if the file unable to open for reading display's error message and stop
		if (!readF)
		{
			cout << "\n ERROR: Unable to open the file student_mech.txt for reading.";
			exit(0);
		}// End of if condition
		...
		for (int c = 0; c < countME; c++)
		{
			...
			// Calls the setter function to set data for current object
			ME[c].setFirstName(firstName);
			ME[c].setLastName(lastName);
			ME[c].setID(id);
			ME[c].setDOB(dob);
			ME[c].setGPA(gpa);
			ME[c].setStartYear(startYear);
			ME[c].setCompletedCredit(completedCredit);
			ME[c].setProgram(program);
		}// End of while loop

		// Close the file
		readF.close();

		// Opens the file for reading CE students information
		readF.open("student_civil.txt");

		// Checks if the file unable to open for reading display's error message and stop
		if (!readF)
		{
			cout << "\n ERROR: Unable to open the file student_civil.txt for reading.";
			exit(0);
		}// End of if condition
		...
		for (int c = 0; c < countCE; c++)
		{
			...
			// Calls the setter function to set data for current object
			CE[c].setFirstName(firstName);
			CE[c].setLastName(lastName);
			CE[c].setID(id);
			CE[c].setDOB(dob);
			CE[c].setGPA(gpa);
			CE[c].setStartYear(startYear);
			CE[c].setCompletedCredit(completedCredit);
			CE[c].setProgram(program);
		}// End of while loop
